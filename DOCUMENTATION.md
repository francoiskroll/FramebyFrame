...
In this case, the overall p-value is the p-value of the F-statistic. Note, this is _exactly_ the p-value of an ANOVA so you can report that way (you can check this for yourself or read e.g. https://faculty.nps.edu/rbassett/_book/regression-with-categorical-variables.html). The only advantage of the linear regression (instead of doing directly an ANOVA) is to obtain an estimate of effect size. 
...
In this situation, the post-hoc p-values is the p-value for each slope from the linear regression. Each p-value is _exactly_ the p-value from a t-test beingGroup vs referenceGroup (you can check this for yourself or see e.g. https://faculty.nps.edu/rbassett/_book/regression-with-categorical-variables.html).
